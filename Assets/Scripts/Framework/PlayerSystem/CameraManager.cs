...
namespace Framework.GamePlay
{
    ...
    ///
    /// 常用用法：
    /// 1) 在场景中新建空物体，添加本脚本（或挂在 PlayerSystem 的根对象上）
    /// 2) 将需要管理的 CinemachineVirtualCamera 拖到 cameras 列表中，并设置唯一 key
    /// 3) 在代码中调用：
    ///    - CameraManager.Instance.SwitchTo("FollowCam");               // 切换到指定相机
    ///    - CameraManager.Instance.SetFollowLookAt(player, playerHead); // 设置跟随与注视目标
    ///    - CameraManager.Instance.Shake(2f, 1.5f, 0.25f);              // 进行短暂相机震动
    ///    - CameraManager.Instance.RegisterVcam(vcam, "TempCam");       // 运行时注册临时相机
    ///    - CameraManager.Instance.UnregisterVcam("TempCam");           // 运行时卸载
    ///
    /// 注意：
    ...
    [AddComponentMenu("PlayerSystem/CameraManager")]
    public class CameraManager : MonoSingleton<CameraManager>
    {
        ...
        void Awake()
        {
            ...
            // 如果已设置默认相机，则切过去
            if (!string.IsNullOrEmpty(CurrentKey) && TryGetVcam(CurrentKey, out var initVcam))
            {
                SwitchTo(initVcam);
            }
            else
            {
                // 如果列表中存在第一个，则默认激活第一个
                if (cameras != null && cameras.Count > 0 && cameras[0].vcam != null)
                {
